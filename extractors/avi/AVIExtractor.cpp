...
struct AVIExtractor::MP3Splitter : public RefBase {
    ...
    status_t read(MediaBufferHelper  **buffer);
...
AVIExtractor::AVISource::AVISource(
        AVIExtractor* extractor, size_t trackIndex)
...
media_status_t AVIExtractor::AVISource::start() {
    ...
    if (8 == mTrack.mBitsPerSample) {
        max_size = mTrack.mMaxSampleSize<<1;
    } else {
        max_size = mTrack.mMaxSampleSize;
    }
    ...
    const char *mime;
    CHECK(AMediaFormat_getString(mTrack.mMeta, AMEDIAFORMAT_KEY_MIME, &mime));

    if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_MPEG)) {
        mSplitter = new MP3Splitter;
        mSplitter->setBufferGroup(mBufferGroup);
    } else {
        mSplitter.clear();
    }
    ...
    for (int i = 0;; i++) {
        if (mSplitter != NULL) {
            ...
            if (err == OK) {
                ...
                AMediaFormat_getInt32(mTrack.mMeta, AMEDIAFORMAT_KEY_CHANNEL_COUNT, &channel_data_old);
/*                if ( channel_data_old != channel_data) {
                   mTrack.mMeta.setInt32(kKeyChannelCount, channel_data);
                }*/
                break;
            } else if (err != -EAGAIN) {
                return (media_status_t) err;
            } else {
                if ( i>100)
               {
                   // to avoid dead loop.
        ...
        if ( !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_RAW)  && (mTrack.mBitsPerSample == 8)) {
            ...
            CHECK_EQ(mBufferGroup->acquire_buffer(&tmp), AMEDIA_OK);
...
void AVIExtractor::MP3Splitter::append(MediaBufferHelper *buffer) {
    ...
    if (mBaseTimeUs < 0) {
        ...
        CHECK(AMediaFormat_getInt64(buffer->meta_data(), AMEDIAFORMAT_KEY_TIME_US, &mBaseTimeUs));
        mNumSamplesRead = 0;
    }
...
status_t AVIExtractor::MP3Splitter::read(MediaBufferHelper **out) {
    *out = NULL;
    ...
    CHECK_EQ(mBufferGroup->acquire_buffer(&mbuf), AMEDIA_OK);
    ...
    AMediaFormat_setInt64(mbuf->meta_data(), AMEDIAFORMAT_KEY_TIME_US, timeUs);

    mBuffer->setRange(
            mBuffer->offset() + frameSize, mBuffer->size() - frameSize);
    mbuf->set_range(mbuf->range_offset(), frameSize);
...
//    ssize_t res = parseChunk(0ll, -1ll);
   ...
   if(mIndexType == NO_INDEX)
    ...
    for (size_t i = 0; i < mTracks.size(); ++i) {
        Track *track = &mTracks.editItemAt(i);
        const char *tmp;
        CHECK(AMediaFormat_getString(track->mMeta, AMEDIAFORMAT_KEY_MIME, &tmp));
...
status_t AVIExtractor::parseStreamFormat(off64_t offset, size_t size) {
    ...
    if (isVideo) {
        ...
        AMediaFormat_setInt32(track->mMeta, AMEDIAFORMAT_KEY_WIDTH, width);
        AMediaFormat_setInt32(track->mMeta, AMEDIAFORMAT_KEY_HEIGHT, height);
    } else {
        ...
        switch(format){
            ...
            case WAVE_FORMAT_PCM:
                if( (16 == bitsPerSample) || (8 == bitsPerSample) ){
                    AMediaFormat_setString(track->mMeta, AMEDIAFORMAT_KEY_MIME, MEDIA_MIMETYPE_AUDIO_RAW);
            ...
            case WAVE_FORMAT_ALAW:
                if(8 == bitsPerSample){
                    AMediaFormat_setString(track->mMeta, AMEDIAFORMAT_KEY_MIME, MEDIA_MIMETYPE_AUDIO_G711_ALAW);
                 }else{
            ...
            case WAVE_FORMAT_MULAW:
                if(8 == bitsPerSample){
                    AMediaFormat_setString(track->mMeta, AMEDIAFORMAT_KEY_MIME, MEDIA_MIMETYPE_AUDIO_G711_MLAW);
                }else{
            ...
            case WAVE_FORMAT_IMAADPCM:
                if(4 == bitsPerSample){
                    AMediaFormat_setString(track->mMeta, AMEDIAFORMAT_KEY_MIME, "audio/ima-adpcm");
                    AMediaFormat_setInt32(track->mMeta, "blka", blockAlign);
                }else{
...
static sp<ABuffer> MakeMPEG4VideoCodecSpecificData(const sp<ABuffer> &config) {
...
status_t AVIExtractor::addMPEG4CodecSpecificData(size_t trackIndex) {
    ...
    for(;;) {
    status_t err =
        getSampleInfo(trackIndex, sampleIndex, &offset, &size, &isKey, &timeUs);
...
status_t AVIExtractor::getSampleIndexAtTime(
    ...
    if (mode == MediaTrackHelper::ReadOptions::SEEK_CLOSEST) {
...
static CreatorFunc Sniff(
    ...
    if (helper.readAt(0, tmp, 12) < 12) {
        return NULL;
    }

    if (!memcmp(tmp, "RIFF", 4) && (!memcmp(&tmp[8], "AVI ", 4) || !memcmp("\x43\x44\x58\x41", &tmp[8], 4))) {
