...
OMX_ERRORTYPE SprdSimpleOMXComponent::freeBuffer(
    ...
    for (size_t i = 0; i < port->mBuffers.size(); ++i) {
        ...
        if (buffer->mHeader == header) {
            ...
            if(header->pInputPortPrivate != NULL) {
...
void SprdSimpleOMXComponent::onChangeState(OMX_STATETYPE state) {
    ...
    {
        ALOGI("mState = %d, mTargetState = %d", mState, mTargetState);
    ...
    case OMX_StateLoaded:
        ...
        {
            ALOGI("OMX_StateLoaded state = %d", state);
            notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
            return;
        }
        break;
    case OMX_StateIdle:
        ...
        {
            ALOGI("OMX_StateIdle state = %d", state);
            notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
            return;
        }
        break;
    case OMX_StateExecuting:
    {
        if(state != OMX_StateIdle)
        {
            ALOGI("OMX_StateExecuting state = %d", state);
            notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
            return;
        }
...
void SprdSimpleOMXComponent::checkTransitions() {
    if (mState != mTargetState) {
        ...
        if (mState == OMX_StateLoaded) {
            ...
            {
                ALOGI("mState = OMX_StateLoaded mTargetState = %d", mTargetState);
                notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
                return;
            }
            for (size_t i = 0; i < mPorts.size(); ++i) {
            ...
            {
                ALOGI("mTargetState = OMX_StateLoaded mState = %d", mState);
                notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
                return;
            }
            for (size_t i = 0; i < mPorts.size(); ++i) {
                ...
                if (n > 0) {
                    ...
                    {
                        ALOGI("n = %d, port.mDef.nBufferCountActual = %d", n , port.mDef.nBufferCountActual);
                        notify(OMX_EventError, OMX_ErrorUndefined, 0, NULL);
                        return;
                    }
                    if (n == port.mDef.nBufferCountActual) {
...
void SprdSimpleOMXComponent:: ConvertFlexYUVToPlanar(
...
void SprdSimpleOMXComponent::drainOneOutputBuffer(OMX_S32 picId, OMX_PTR pBufferHeader, OMX_U64 pts) {
}
