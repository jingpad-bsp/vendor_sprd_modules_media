...
{
#if defined(USE_AVC_DECODE_COEXISTENCE)
   AddCore("libomx_coexist_core.so");
#else
   AddCore("libsprd_omx_core.so");
#if defined(USE_MEDIASDK)
   AddCore("libmfx_omx_core.so");
#endif //#if defined(USE_MEDIASDK)
#endif //#if defined(USE_AVC_DECODE_COEXISTENCE)
   ...
   if ((property_get("ro.vendor.media.nocorefile.enabled", value, "false") != 0) && !strcmp(value, "true")) {
       rlimit rl;
       rl.rlim_cur = 0;
       rl.rlim_max = 0;
       ALOGI("set nocorefile for mediacodec before");
       if(prlimit(getpid(),4,&rl,NULL) == 0) {
           ALOGE("prlimit(RLIMIT_CORE) failed for pid %d",getpid());
       }
       ALOGI("set nocorefile for mediacodec end");
   }

}

OMX_ERRORTYPE SprdOMXPlugin::AddCore(const char* coreName)
{
   void* libHandle = dlopen(coreName, RTLD_NOW);

   if (libHandle != NULL) {
        SprdOMXCore* core = (SprdOMXCore*)calloc(1,sizeof(SprdOMXCore));
        ...
        core->mInit = (SprdOMXCore::InitFunc)dlsym(libHandle, "OMX_Init");
        core->mDeinit = (SprdOMXCore::DeinitFunc)dlsym(libHandle, "OMX_Deinit");

        core->mComponentNameEnum =
            (SprdOMXCore::ComponentNameEnumFunc)dlsym(libHandle, "OMX_ComponentNameEnum");

        core->mGetHandle = (SprdOMXCore::GetHandleFunc)dlsym(libHandle, "OMX_GetHandle");
        core->mFreeHandle = (SprdOMXCore::FreeHandleFunc)dlsym(libHandle, "OMX_FreeHandle");

        core->mGetRolesOfComponentHandle =
            (SprdOMXCore::GetRolesOfComponentFunc)dlsym(
