...
SPRDDeinterlace::SPRDDeinterlace(void* decOutputBufQueue, void* deinterInputBufQueue, void* outQueue, DeinterlaceInfo& info) :
    mDone(false) ,
    mDeintlFrameNum(0),
    mIOMMU_VPP_Enabled(false),
    mIOMMU_VPP_ID(-1),
    mDecOutputBufQueue((List<BufferInfoBase *>*)decOutputBufQueue),
    mDeinterInputBufQueue((List<BufferInfoBase *>*)deinterInputBufQueue),
    mOutQueue((List<BufferInfoBase *>*)outQueue){

    ALOGI("Construct SPRDDeinterlace, this: %p", (void *)this);
    //mComponent = component;
    //mDispBufferCtrl = new BufferCtrlStruct;
    memcpy(&mDeinterlaceInfo, &info, sizeof(DeinterlaceInfo));
...
void SPRDDeinterlace::fillDeintlSrcBuffer(/*SprdSimpleOMXComponent* component*/
        BufferInfoBase* itBufferNodePre, BufferInfoBase * itBufferNodeCur
         ) {
   /* ALOGV("%s, %d: mDeinterInputBufQueue size: %d, mDecOutputBufQueue size: %d, mDeintlFrameNum: %d\n",
    ...
    if(mDeintlFrameNum > 1) {
        ...
        mDecOutputBufQueue->push_back(itBufferNodePre);
        ...
        if ((itBufferNodeCur->mHeader->nFlags == OMX_BUFFERFLAG_EOS)) {
            ...
            mDecOutputBufQueue->push_back(itBufferNodeCur);
            mDeinterInputBufQueue->erase(mDeinterInputBufQueue->begin());
            ALOGE("%s, %d: Deinterlace meet EOS, mDeinterInputBufQueue size: %d, mDecOutputBufQueue size: %d\n",
                  __FUNCTION__, __LINE__, mDeinterInputBufQueue->size(), mDecOutputBufQueue->size());
        }
    }
...
void SPRDDeinterlace::deintlThreadFunc() {
   ...
   // List<SprdSimpleOMXComponent::BufferInfo *> &outQueue = component->getPortQueue(kOutputPortIndex);
    ...
    BufferInfoBase * itBufferNodePre =NULL;
    BufferInfoBase * itBufferNodeCur = NULL;
    ...
    while (!mDone) {
        Mutex::Autolock autoLock(*mDeinterlaceInfo.threadLock);

        while ( mOutQueue->empty() || mDone || mDeinterInputBufQueue->empty()
                || (mDeinterInputBufQueue->size() < 2 && mDeintlFrameNum > 0)) {
            if (mDone) {
                ALOGI("%s, %d: deintl thread done\n", __FUNCTION__, __LINE__);
                return;
            } else {
                ALOGI("%s, %d, mDeinterReadyCondition wait\n", __FUNCTION__, __LINE__);
                mDeinterReadyCondition.wait(*mDeinterlaceInfo.threadLock);
            }
        }

        ALOGI("%s, %d, start to do deinterlace, mDeintlFrameNum = %d\n", __FUNCTION__, __LINE__, mDeintlFrameNum);
        List<BufferInfoBase *>::iterator itor = mDeinterInputBufQueue->begin();
        ...
        List<BufferInfoBase *>::iterator itBuffer = mOutQueue->begin();
        ...
        if(mUseNativeBuffer) {
            ...
            int width = mDeinterlaceInfo.width;
            int height = mDeinterlaceInfo.height;
        ...
        if(ret == 0) {
            ...
            mDeinterlaceInfo.callback(mDeinterlaceInfo.decoder, mDeintlFrameNum, outHeader, (unsigned long long)outHeader->nTimeStamp);
            //component->drainOneOutputBuffer(mDeintlFrameNum, outHeader, (uint64)outHeader->nTimeStamp);
        }

        fillDeintlSrcBuffer(itBufferNodePre, itBufferNodeCur);
    }
...
void SPRDDeinterlace::stopDeinterlaceThread() {
    ALOGI("stopDeinterlaceThread B");
    mDone = true;
    mDeinterReadyCondition.signal();

    void *dummy;
    pthread_join(mThread, &dummy);
    ALOGI("stopDeinterlaceThread E");
