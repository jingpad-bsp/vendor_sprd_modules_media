...
#else
    val = V_BIT_30 | V_BIT_28 |V_BIT_26 | V_BIT_24;
#endif
...
#else
    if (p_deint_params->interleave) {
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG2_OFF, (src_frame_addr) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG3_OFF, (src_frame_addr + y_len) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG4_OFF, 0, "");

        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG5_OFF, (dst_frame_addr) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG6_OFF, (dst_frame_addr + y_len) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG7_OFF, 0, "");

        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG8_OFF, (ref_frame_addr) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG9_OFF, (ref_frame_addr + y_len) >>4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG10_OFF, 0, "");
    } else {
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG2_OFF, (src_frame_addr) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG3_OFF, (src_frame_addr + y_len) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG4_OFF, (src_frame_addr + y_len + c_len) >> 4, "");

        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG5_OFF, (dst_frame_addr) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG6_OFF, (dst_frame_addr + y_len) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG7_OFF, (dst_frame_addr + y_len + c_len) >>4, "");

        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG8_OFF, (ref_frame_addr) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG9_OFF, (ref_frame_addr + y_len) >> 4, "");
        VSP_WRITE_REG(GLB_REG_BASE_ADDR+VSP_CFG10_OFF, (ref_frame_addr + y_len + c_len) >> 4, "");
    }
#endif
    return 0;
}
...
int32 vpp_deint_process(VPPObject *vo, unsigned long src_frame,
...
#else
    {
        ...
        int count = 20;
#ifndef VPP_SHARKL3_DEINT
        while (((cmd & V_BIT_9) == 0) && count--) {
#else
        while (((cmd & V_BIT_11) == 0) && count--) {
#endif
            usleep(1000);
