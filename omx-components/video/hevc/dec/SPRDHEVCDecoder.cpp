...
SPRDHEVCDecoder::SPRDHEVCDecoder(
    ...
    property_get("vendor.h265dec.yuv.dump", value_dump, "false");
    mDumpYUVEnabled = !strcmp(value_dump, "true");

    property_get("vendor.h265dec.strm.dump", value_dump, "false");
    ...
    if(mDumpStrmEnabled){
        char s2[100];
        sprintf(s2,"/data/misc/media/video_es_%p_%lld.h265",(void *)this,start_decode);
...
//       dump_bs( mPbuf_stream_v, dec_in.dataLen);
        ...
        if(mDumpYUVEnabled){
            if (mFile_yuv != NULL) {
                fwrite(dec_out.pOutFrameY, 1, mPictureSize, mFile_yuv);
            }
        }
...
bool SPRDHEVCDecoder::handlePortSettingChangeEvent(const H265SwDecInfo *info) {
    ...
    if ((info->picWidth & 31) && (mFbcMode == AFBC)) {
        mFbcMode = FBC_NONE;
        mUsage = GRALLOC_USAGE_SW_READ_OFTEN | GRALLOC_USAGE_SW_WRITE_OFTEN;
...
void SPRDHEVCDecoder::updateFinalColorAspects(
...
status_t SPRDHEVCDecoder::handleColorAspectsChange() {
