...
SPRDMPEG4Decoder::SPRDMPEG4Decoder(
    ...
    : SprdSimpleOMXComponent(name, callbacks, appData, component),
      ...
      mMP4DecGetIOVA(NULL),
      mMP4DecFreeIOVA(NULL),
      mMP4DecGetIOMMUStatus(NULL),
      mMp4NeedToSwitchToSwDecoder(NULL),
      mFrameDecoded(false) {
    ...
    property_get("vendor.m4vdec.yuv.dump", value_dump, "false");
    mDumpYUVEnabled = !strcmp(value_dump, "true");

    property_get("vendor.m4vdec.strm.dump", value_dump, "false");
    ...
    if(mDumpStrmEnabled){
        char s2[100];
        sprintf(s2,"/data/misc/media/video_es_%p_%lld.m4v",(void *)this,start_decode);
...
status_t SPRDMPEG4Decoder::initDecoder() {
    ...
    int ret = (*mMP4DecGetIOMMUStatus)(mHandle);
    ...
    if (mDecoderSwFlag) {
        ...
        if (fd < 0) {
            ...
            int ret;
            if (mIOMMUEnabled) {
                ret = (*mMP4DecGetIOVA)(mHandle, fd, &phy_addr, &size);
            } else {
...
void SPRDMPEG4Decoder::releaseDecoder() {
    if (mPbuf_stream_v != NULL) {
        if (mDecoderSwFlag) {
            ...
            if (mIOMMUEnabled) {
                (*mMP4DecFreeIOVA)(mHandle, mPbuf_stream_p, mPbuf_stream_size);
            }
            mPmem_stream.clear();
            mPbuf_stream_v = NULL;
            mPbuf_stream_p = 0;
    ...
    if(mPbuf_extra_v != NULL) {
        if (mIOMMUEnabled) {
            (*mMP4DecFreeIOVA)(mHandle, mPbuf_extra_p, mPbuf_extra_size);
        }
        mPmem_extra.clear();
        mPbuf_extra_v = NULL;
        mPbuf_extra_p = 0;
...
OMX_ERRORTYPE SPRDMPEG4Decoder::internalSetParameter(
    ...
    case OMX_IndexParamPortDefinition:
    {
        ...
        if(defParams->nPortIndex == 1) {
            ...
            port->mDef.nBufferSize += (mFbcMode == AFBC)? ((mWidth*mHeight/16 + 1023) & (~1023)):0;
        }
    ...
    if(portIndex == OMX_DirOutput) {
        ...
        if(mAllocateBuffers) {
            ...
            if (mIOMMUEnabled) {
                ...
                (*mMP4DecGetIOVA)(mHandle, fd, &picPhyAddr, &bufferSize);
...
OMX_ERRORTYPE SPRDMPEG4Decoder::allocateBuffer(
    ...
    case OMX_DirOutput:
    {
        ...
        if(mDecoderSwFlag  || mChangeToSwDec) {
            ...
            if (mIOMMUEnabled) {
                ret = (*mMP4DecGetIOVA)(mHandle, fd, &phyAddr, &bufferSize);
                if(ret) {
            ...
            ALOGI("allocateBuffer, allocate buffer from pmem, pBuffer: 0x%p, phyAddr: 0x%lx, size: %zd, fd:%d", pBuffer, phyAddr, 
                bufferSize, fd);
    ...
    case OMX_DirOutput:
    {
        ...
        if(pBufCtrl != NULL) {
            if (mIOMMUEnabled && (pBufCtrl->phyAddr > 0)) {
                ...
                (*mMP4DecFreeIOVA)(mHandle, pBufCtrl->phyAddr, pBufCtrl->bufferSize);
            }
...
void SPRDMPEG4Decoder::onQueueFilled(OMX_U32 portIndex) {
    ...
    if(mChangeToSwDec) {
        ...
        /*when changetoswdec IOMMU is not need,and must be first to freeiova before open swlib*/
        if (mIOMMUEnabled) {
    ...
    while (!mStopDecode && (mEOSStatus != INPUT_DATA_AVAILABLE || !inQueue.empty())
        ...
        ALOGI("%s, %d, outHeader:0x%p, inHeader: 0x%p, len: %d, time: %lld, EOS: %d, cfg:%d",
                __FUNCTION__, __LINE__,outHeader, inHeader, inHeader->nFilledLen, inHeader->nTimeStamp,
                inHeader->nFlags & OMX_BUFFERFLAG_EOS, inHeader->nFlags & OMX_BUFFERFLAG_CODECCONFIG);
        ...
        if (!mInitialized) {
            ...
            video_format.thumbnail = mThumbnailMode;
        ...
        ALOGI("%s, %d, decRet: %d, %dms, frameEffective: %d, pOutFrameY: %0x, pBufferHeader: %0x, needIVOP: %d, error_flag: %0x",
              __FUNCTION__, __LINE__, decRet, (unsigned int)((end_decode-start_decode) / 1000000L),dec_out.frameEffective,
              dec_out.pOutFrameY, dec_out.pBufferHeader,mNeedIVOP, mHandle->g_mpeg4_dec_err_flag);
...
bool SPRDMPEG4Decoder::portSettingsChanged() {
    ...
    if (((buf_width & 31) || (buf_height & 7) ||
            mChangeToSwDec || mDecoderSwFlag) && (mFbcMode == AFBC)) {
        mFbcMode = FBC_NONE;
        mUsage = GRALLOC_USAGE_SW_READ_OFTEN | GRALLOC_USAGE_SW_WRITE_OFTEN;
        notify(OMX_EventPortSettingsChanged, 1, 0, NULL);
        mOutputPortSettingsChange = AWAITING_DISABLED;
        ALOGI("width %d, change2sw %d, swflag %d, disable AFBC\n", buf_width, mChangeToSwDec, mDecoderSwFlag);
        return true;
    }
    ...
    if(!mDecoderSwFlag) {
        ...
        if(mMp4NeedToSwitchToSwDecoder != NULL)
            if(mMp4NeedToSwitchToSwDecoder(mHandle)) {
                mChangeToSwDec = true;
                ret = true;
            }
    }
...
void SPRDMPEG4Decoder::updatePortDefinitions() {
    ...
    def->nBufferSize += (mFbcMode == AFBC)? ((mWidth*mHeight/16 + 1023) & (~1023)):0;
...
int SPRDMPEG4Decoder::extMemoryAlloc(unsigned int extra_mem_size) {
    ...
    if (mDecoderSwFlag) {
        ...
        if(fd >= 0) {
            ...
            if (mIOMMUEnabled) {
                ret = (*mMP4DecGetIOVA)(mHandle, fd, &phy_addr, &buffer_size);
            } else {
...
bool SPRDMPEG4Decoder::openDecoder(const char* libName) {
    ...
    mMP4DecGetIOVA = (FT_MP4Dec_get_iova)dlsym(mLibHandle, "MP4Dec_get_iova");
    if(mMP4DecGetIOVA == NULL) {
        ALOGE("Can't find MP4Dec_get_iova in %s",libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }

    mMP4DecFreeIOVA = (FT_MP4Dec_free_iova)dlsym(mLibHandle, "MP4Dec_free_iova");
    if(mMP4DecFreeIOVA == NULL) {
        ALOGE("Can't find MP4Dec_free_iova in %s",libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }

    mMP4DecGetIOMMUStatus = (FT_MP4Dec_get_IOMMU_status)dlsym(mLibHandle, "MP4Dec_get_IOMMU_status");
    if(mMP4DecGetIOMMUStatus == NULL) {
        ALOGE("Can't find MP4Dec_get_IOMMU_status in %s",libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }

    mMp4NeedToSwitchToSwDecoder = (FT_Mp4NeedToSwitchToSwDecoder)dlsym(mLibHandle, "Mp4NeedToSwitchToSwDecoder");
    if(mMp4NeedToSwitchToSwDecoder== NULL && strcmp(libName,"libomx_m4vh263dec_sw_sprd.so")) {
        ALOGE("Can't find Mp4NeedToSwitchToSwDecoder in %s",libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }
...
void SPRDMPEG4Decoder::freeOutputBufferIOVA() {
