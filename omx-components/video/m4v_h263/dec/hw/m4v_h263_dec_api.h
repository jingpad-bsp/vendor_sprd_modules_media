...
typedef enum {
  INTER_MEM = 0,  /*internal memory, only for software writing and reading and
                     initialized when initialize decoder*/
  HW_NO_CACHABLE, /*physical continuous and no-cachable, only for VSP writing
                     and reading */
  HW_CACHABLE, /*physical continuous and cachable, for software writing and VSP
                  reading */
...
typedef struct {
  ...
  unsigned long
      pStream_phy;  // Pointer to stream to be decoded. Physical address.
...
typedef int (*FunctionType_MemAllocCB)(void *userData,
                                       unsigned int extra_mem_size);
...
void Mp4GetVideoDimensions(MP4Handle *mp4Handle, int32 *display_width,
                           int32 *display_height);
void Mp4GetBufferDimensions(MP4Handle *mp4Handle, int32 *width, int32 *height);
MMDecRet MP4GetCodecCapability(MP4Handle *mp4Handle, int32 *max_width,
                               int32 *max_height);
 int Mp4NeedToSwitchToSwDecoder(MP4Handle *mp4Handle);
...
MMDecRet MP4DecVolHeader(MP4Handle *mp4Handle,
                         MMDecVideoFormat *video_format_ptr);
...
//Note:
/*****************************************************************************/
MMDecRet MP4DecDecode(MP4Handle *mp4Handle, MMDecInput *pInput,
                      MMDecOutput *pOutput);
...
int MP4Dec_get_iova(MP4Handle *mp4Handle, int fd, unsigned long *iova, size_t *size);

int MP4Dec_free_iova(MP4Handle *mp4Handle, unsigned long iova, size_t size);

int MP4Dec_get_IOMMU_status(MP4Handle *mp4Handle);
...
typedef void (*FT_MP4DecSetCurRecPic)(MP4Handle *mp4Handle, uint8 *pFrameY,
                                      uint8 *pFrameY_phy, void *pBufferHeader);
typedef void (*FT_MP4DecSetReferenceYUV)(MP4Handle *mp4Handle, uint8 *pFrameY);
typedef void (*FT_Mp4GetVideoDimensions)(MP4Handle *mp4Handle,
                                         int32 *display_width,
                                         int32 *display_height);
typedef void (*FT_Mp4GetBufferDimensions)(MP4Handle *mp4Handle, int32 *width,
                                          int32 *height);
typedef MMDecRet (*FT_MP4GetCodecCapability)(MP4Handle *mp4Handle,
                                             int32 *max_width,
                                             int32 *max_height);

typedef MMDecRet (*FT_MP4DecInit)(MP4Handle *mp4Handle, MMCodecBuffer *pBuffer);
typedef MMDecRet (*FT_MP4DecVolHeader)(MP4Handle *mp4Handle,
                                       MMDecVideoFormat *video_format_ptr);
typedef MMDecRet (*FT_MP4DecMemInit)(MP4Handle *mp4Handle,
                                     MMCodecBuffer *pBuffer);
typedef MMDecRet (*FT_MP4DecDecode)(MP4Handle *mp4Handle, MMDecInput *pInput,
                                    MMDecOutput *pOutput);
typedef MMDecRet (*FT_MP4DecRelease)(MP4Handle *mp4Handle);

typedef int (*FT_MP4Dec_get_iova)(MP4Handle *mp4Handle, int fd, unsigned long *iova, size_t *size);

typedef int (*FT_MP4Dec_free_iova)(MP4Handle *mp4Handle, unsigned long iova, size_t size);

typedef int (*FT_MP4Dec_get_IOMMU_status)(MP4Handle *mp4Handle);
typedef int (*FT_Mp4NeedToSwitchToSwDecoder)(MP4Handle *mp4Handle);
