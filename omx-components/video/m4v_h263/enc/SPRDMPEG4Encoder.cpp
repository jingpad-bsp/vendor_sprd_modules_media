...
SPRDMPEG4Encoder::SPRDMPEG4Encoder(
    ...
    : SprdSimpleOMXComponent(name, callbacks, appData, component),
      ...
      mMP4EncRelease(NULL),
      mMP4EncGetIOVA(NULL),
      mMP4EncFreeIOVA(NULL),
      mMP4EncGetIOMMUStatus(NULL),
      mMP4Enc_NeedAlign(NULL),
      mMP4Enc_ExtraMemSize(NULL),
      mFrameWidth(176),
      mFrameHeight(144),
      mSupportRGBEnc(false) {

    ALOGI("Construct SPRDMPEG4Encoder, this: 0x%p, instances: %d", (void *)this, instances);
    ...
    if(openEncoder("libomx_m4vh263enc_hw_sprd.so")==false){
        CHECK_EQ(openEncoder("libomx_m4vh263enc_sw_sprd.so"),true);
        mCacheType = 0;
        mCacheMask = 1<<31;
        mEncoderSwFlag = true;
    }
    else{
        mCacheType = MemIon::NO_CACHING;
        mCacheMask = 0;
        mEncoderSwFlag = false;
    }
    ...
    property_get("vendor.m4venc.yuv.dump", value_dump, "false");
    mDumpYUVEnabled = !strcmp(value_dump, "true");

    property_get("vendor.m4venc.strm.dump", value_dump, "false");
    ...
    if(mEncoderSwFlag==false)
        mSupportRGBEnc = true;
#endif
    int ret = (*mMP4Enc_NeedAlign)(mHandle);
    ALOGI("Enc needAign:%d",ret);
    if (ret == 0) {
        mNeedAlign = false;
    }
    ret = (*mMP4EncGetIOMMUStatus)(mHandle);
   ...
   instances++;
    ...
    if(mDumpYUVEnabled){
        char s1[100];
        sprintf(s1,"/data/misc/media/video_in_%p_%lld.yuv",(void *)this,start_encode);
    ...
    if(mDumpStrmEnabled){
        char s2[100];
        sprintf(s2,"/data/misc/media/video_es_%p_%lld.m4v",(void *)this,start_encode);
...
SPRDMPEG4Encoder::~SPRDMPEG4Encoder() {
    ALOGI("Destruct SPRDMPEG4Encoder, this: 0x%p, instances: %d", (void *)this, instances);
...
OMX_ERRORTYPE SPRDMPEG4Encoder::initCheck() const {
...
OMX_ERRORTYPE SPRDMPEG4Encoder::initEncParams() {
    ...
    size_extra = (*mMP4Enc_ExtraMemSize)(mHandle,mVideoWidth,mVideoHeight);
    ALOGI("size_extra %d",size_extra);
...
#endif
    if (mIOMMUEnabled) {
        mPmem_extra = new MemIon("/dev/ion", size_extra,mCacheType, mCacheMask |ION_HEAP_ID_MASK_SYSTEM);
    } else {
        mPmem_extra = new MemIon("/dev/ion", size_extra,mCacheType, mCacheMask |ION_HEAP_ID_MASK_MM);
    }
    ...
    if (fd < 0) {
        ...
        int ret;
        if (mIOMMUEnabled) {
            ret = (*mMP4EncGetIOVA)(mHandle, fd, &phy_addr, &size);
        } else {
    ...
    if (mIOMMUEnabled) {
        mPmem_stream = new MemIon("/dev/ion", size_stream,mCacheType, mCacheMask |ION_HEAP_ID_MASK_SYSTEM);
    } else {
        mPmem_stream = new MemIon("/dev/ion", size_stream,mCacheType, mCacheMask |ION_HEAP_ID_MASK_MM);
    }
...
OMX_ERRORTYPE SPRDMPEG4Encoder::releaseEncoder() {
    ...
    if(mEncoderSwFlag==true)
        (*mMP4EncRelease)(mHandle);

    if (mPbuf_extra_v != NULL) {
        if (mIOMMUEnabled) {
            (*mMP4EncFreeIOVA)(mHandle, mPbuf_extra_p, mPbuf_extra_size);
        }
        mPmem_extra.clear();
        mPbuf_extra_v = NULL;
        mPbuf_extra_p = 0;
    ...
    if (mPbuf_stream_v != NULL) {
        if (mIOMMUEnabled) {
            (*mMP4EncFreeIOVA)(mHandle, mPbuf_stream_p, mPbuf_stream_size);
        }
        mPmem_stream.clear();
        mPbuf_stream_v = NULL;
        mPbuf_stream_p = 0;
    ...
    if (mPbuf_yuv_v != NULL) {
        if (mIOMMUEnabled) {
            (*mMP4EncFreeIOVA)(mHandle, mPbuf_yuv_p, mPbuf_yuv_size);
        }
...
void SPRDMPEG4Encoder::initPorts() {
    ...
    def.format.video.cMIMEType =
        (mIsH263 == 0)
        ? const_cast<char *>("video/mp4v-es")
        : const_cast<char *>("video/3gpp");
...
void SPRDMPEG4Encoder::onQueueFilled(OMX_U32 portIndex) {
    ...
    List<BufferInfo *> &inQueue = getPortQueue(0);
    List<BufferInfo *> &outQueue = getPortQueue(1);
    ALOGI("%s,%d,in queue size %d,out queue size %d",__FUNCTION__,__LINE__,inQueue.size(),outQueue.size());
        ...
        if (inHeader->nFilledLen > 0) {
            ...
            if (mStoreMetaData) {
                ...
                if (type == kMetadataBufferTypeCameraSource) {
                    ...
                    int ret = 0;
                    if (mIOMMUEnabled) {
                        ret = (*mMP4EncGetIOVA)(mHandle, fd, &py_addr, &buf_size);
                    } else {
                    ...
                    if (mPbuf_yuv_v == NULL) {
                        ...
                        if (mIOMMUEnabled) {
                            mYUVInPmemHeap = new MemIon("/dev/ion", yuv_size,mCacheType, mCacheMask | ION_HEAP_ID_MASK_SYSTEM);
                        } else {
                            mYUVInPmemHeap = new MemIon("/dev/ion", yuv_size,mCacheType, mCacheMask | ION_HEAP_ID_MASK_MM);
                        }
                        ...
                        if (mIOMMUEnabled) {
                            ret = (*mMP4EncGetIOVA)(mHandle, fd, &phy_addr, &buffer_size);
                        } else {
                    ...
                    if (mVideoColorFormat == OMX_COLOR_FormatYUV420Planar) {
                        ...
                        if(ADP_FORMAT(buf) == HAL_PIXEL_FORMAT_YCrCb_420_SP ||
                            ...
                            ConvertARGB888ToYVU420SemiPlanar((uint8_t*)vaddr, py, mVideoWidth, mVideoHeight, mVideoWidth, mVideoHeight);
                        }
                    } else {
                        if (ADP_USAGE(buf) & GRALLOC_USAGE_HW_VIDEO_ENCODER) {
                            ...
                            if (mIOMMUEnabled) {
                                if ((*mMP4EncGetIOVA)(mHandle, fd, &ion_addr, &ion_size)) {
                                    ALOGE("%s, %d, mMP4EncGetIOVA error", __FUNCTION__, __LINE__);
                                    return;
                                }
                                needUnmap = true;
                                bufFd = fd;
                                ...
                                if (0 != MemIon::Get_phy_addr_from_ion(fd, &ion_addr, &ion_size)) {
                                    ALOGE("%s, %d, Get_phy_addr_from_ion error", __FUNCTION__, __LINE__);
                                    return;
                                }
                            }
                ...
                if (mVideoColorFormat == OMX_COLOR_FormatYUV420Planar) {
                    ...
                    if (mSupportRGBEnc) {
                        ...
                        ConvertARGB888ToYVU420SemiPlanar(inputData, py, mVideoWidth, mVideoHeight, mVideoWidth, mVideoHeight);
                    }
                } else {
            ...
            int64_t end_encode = systemTime();
            ALOGI("%s,%d,MP4EncStrmEncode[%lld] %dms, in {%p-%p, %dx%d, %d}, out {%p-%d, %d}, wh{%d, %d}, xy{%d, %d}",__FUNCTION__,__LINE__,
            ...
            if (needUnmap) {
                ...
                (*mMP4EncFreeIOVA)(mHandle, iova, iovaLen);
            }
...
bool SPRDMPEG4Encoder::openEncoder(const char* libName) {
    ...
    mMP4EncGetIOVA = (FT_MP4Enc_get_iova)dlsym(mLibHandle, "MP4Enc_get_iova");
    if (mMP4EncGetIOVA == NULL) {
        ALOGE("Can't find MP4Enc_get_iova in %s", libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }

    mMP4EncFreeIOVA = (FT_MP4Enc_free_iova)dlsym(mLibHandle, "MP4Enc_free_iova");
    if (mMP4EncFreeIOVA == NULL) {
        ALOGE("Can't find MP4Enc_free_iova in %s", libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }

    mMP4EncGetIOMMUStatus = (FT_MP4Enc_get_IOMMU_status)dlsym(mLibHandle, "MP4Enc_get_IOMMU_status");
    if (mMP4EncGetIOMMUStatus == NULL) {
        ALOGE("Can't find MP4Enc_get_IOMMU_status in %s", libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }

    mMP4Enc_NeedAlign = (FT_MP4Enc_Need_Align)dlsym(mLibHandle, "MP4Enc_NeedAlign");
    if(mMP4Enc_NeedAlign == NULL) {
        ALOGE("Can't find MP4Enc_NeedAlign in %s",libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }

    mMP4Enc_ExtraMemSize = (FT_MP4Enc_ExtraMemSize)dlsym(mLibHandle, "MP4Enc_ExtraMemSize");
    if(mMP4Enc_ExtraMemSize == NULL) {
        ALOGE("Can't find MP4Enc_ExtraMemSize in %s",libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }
