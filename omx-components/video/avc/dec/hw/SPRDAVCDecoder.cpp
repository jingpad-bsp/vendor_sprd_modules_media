...
SPRDAVCDecoder::SPRDAVCDecoder(
    ...
    : SprdSimpleOMXComponent(name, callbacks, appData, component),
      ...
      mH264DecInitStructForNewSeq(NULL),
      mH264DecNeedToSwitchToSwDecoder(NULL),
      mFrameDecoded(false),
...
#define USE_HW_DECODER 0x00
    ...
    property_get("vendor.h264dec.yuv.dump", value_dump, "false");
    mDumpYUVEnabled = !strcmp(value_dump, "true");

    property_get("vendor.h264dec.strm.dump", value_dump, "false");
...
OMX_ERRORTYPE SPRDAVCDecoder::internalSetParameter(
    ...
    case OMX_IndexParamPortDefinition:
    {
        ...
        if (!((newWidth <= mCapability.max_width && newHeight <= mCapability.max_height)
            || (newWidth <= mCapability.max_height && newHeight <= mCapability.max_width))) {
            ALOGE("%s %d, [%d,%d] is out of range [%d, %d], failed to support this format.",
                __FUNCTION__, __LINE__, newWidth, newHeight, mCapability.max_width, mCapability.max_height);
            return OMX_ErrorUnsupportedSetting;
        }
...
OMX_ERRORTYPE SPRDAVCDecoder::getConfig(
    ...
    case OMX_IndexDescribeColorAspects:
    {
...
#endif
        ALOGI("%s,%d,setConfig, mDecSceneMode = %d",__FUNCTION__,__LINE__, mDecSceneMode);
        return OMX_ErrorNone;
    }
...
void SPRDAVCDecoder::onQueueFilled(OMX_U32 portIndex) {
    ...
    if(mChangeToSwDec) {
        ...
        releaseDecoder();
        if(3 == mDecSceneMode){// for VoLTE mode
            if(!openDecoder("libomx_avcdec_vt_sprd.so")){
                ALOGE("onQueueFilled, open  libomx_avcdec_vt_sprd.so failed.");
        ...
        else if(!openDecoder("libomx_avcdec_sw_sprd.so")) {
    ...
    while (!mStopDecode && (mEOSStatus != INPUT_DATA_AVAILABLE || !inQueue.empty())
        ...
        if (!mSecureFlag) {
            if(mThumbnailMode) {
                uint32_t  iIndex = 0;
                uint32_t  iRemainedBitstream;
                iRemainedBitstream = inHeader->nFilledLen - inHeader->nOffset;
                while ((bitstream[iIndex] == 0x0) && (iIndex < iRemainedBitstream)){
                    iIndex++;
                }
                if (iIndex < 2 || (bitstream[iIndex] != 0x1)){
...
bool SPRDAVCDecoder::handlePortSettingChangeEvent(const H264SwDecInfo *info) {
    ...
    if (!mDecoderSwFlag) {
        if(mH264DecNeedToSwitchToSwDecoder != NULL)
            if(mH264DecNeedToSwitchToSwDecoder(mHandle)) {
                mChangeToSwDec = true;
            }
    }
    if ((((info->picWidth & 31) || (!info->frame_mb_only))
        || mChangeToSwDec || mDecoderSwFlag) && (mFbcMode == AFBC)) {
        mFbcMode = FBC_NONE;
        mUsage = GRALLOC_USAGE_SW_READ_OFTEN | GRALLOC_USAGE_SW_WRITE_OFTEN;
        notify(OMX_EventPortSettingsChanged, 1, 0, NULL);
        mOutputPortSettingsChange = AWAITING_DISABLED;
        ALOGI("width %d, change2sw %d, swflag %d,frame_mb_only %d, disable AFBC\n", info->picWidth, mChangeToSwDec, mDecoderSwFlag, info->frame_mb_only);
        return true;
    }
    ...
    if ((mStride != info->picWidth) || (mSliceHeight != info->picHeight) || cropChanged || (!mThumbnailMode && needFlushBuffer)) {
        ...
        notify(OMX_EventPortSettingsChanged, kOutputPortIndex,
                OMX_IndexDescribeColorAspects, NULL);
        mUpdateColorAspects = false;
    }
...
bool SPRDAVCDecoder::drainAllOutputBuffers() {
    ...
    while (mEOSStatus != OUTPUT_FRAMES_FLUSHED) {
        ...
        if(mDeintl) {
            ...
            if (mDeinterInputBufQueue.size() == 0) {
                ALOGI("mDeinterInputBufQueue.size is 0");
                notifyFillBufferDone(outHeader);
            } else {
...
void SPRDAVCDecoder::onPortFlushPrepare(OMX_U32 portIndex) {
    ...
    if(portIndex == OMX_DirOutput) {
        ...
        if ( NULL != mH264DecInitStructForNewSeq)
...
// static
void  SPRDAVCDecoder::DrainOneOutputBuffercallback(void* decoder, int32_t picId, void* pBufferHeader, unsigned long long pts)
{
    return static_cast<SPRDAVCDecoder *>(decoder)->drainOneOutputBuffer(picId, pBufferHeader, pts);
}
...
OMX_ERRORTYPE SPRDAVCDecoder::getExtensionIndex(
    ...
    }else if (!strcmp(name, SPRD_INDEX_DESCRIBE_COLOR_ASPECTS)
                && supportsDescribeColorAspects()) {
        *(int32_t*)index = OMX_IndexDescribeColorAspects;
        return OMX_ErrorNone;
...
bool SPRDAVCDecoder::openDecoder(const char* libName) {
    ...
    mH264DecInitStructForNewSeq = (FT_H264Dec_InitStructForNewSeq)dlsym(mLibHandle, "H264Dec_InitStructForNewSeq");
    if(mH264DecInitStructForNewSeq == NULL) {
        ALOGE("Can't find H264Dec_InitStructForNewSeq in %s",libName);
        dlclose(mLibHandle);
        mLibHandle = NULL;
        return false;
    }
    if(!strcmp(libName,"libomx_avcdec_hw_sprd.so")){
        mH264DecNeedToSwitchToSwDecoder = (FT_H264DecNeedToSwitchToSwDecoder)dlsym(mLibHandle, "H264DecNeedToSwitchToSwDecoder");
        if(mH264DecNeedToSwitchToSwDecoder== NULL) {
            ALOGE("Can't find H264NeedToSwitchToSwDecoder in %s",libName);
            dlclose(mLibHandle);
            mLibHandle = NULL;
            return false;
        }
        ...
        if (decRet == MMDEC_OK) {
            if (nal_type == 0x7/*SPS*/ &&
                ...
                if ((*mH264DecInitStructForNewSeq)(mHandle) != MMDEC_OK) {
                    ALOGE("Failed to InitStructForNewSeq");
                }
...
void SPRDAVCDecoder::updateFinalColorAspects(
...
status_t SPRDAVCDecoder::handleColorAspectsChange() {
    int perference = getColorAspectPreference();
    ALOGV("Color Aspects preference: %d ", perference);
...
bool SPRDAVCDecoder::getVUIParams(H264SwDecInfo *mDecoderInfo) {

    int32_t primaries = mDecoderInfo->colorPrimaries;
    int32_t transfer = mDecoderInfo->transfer;
    int32_t coeffs = mDecoderInfo->matrixCoefficients;
    bool fullRange = mDecoderInfo->videoRange;
    ALOGV("primaris :%d,transfer:%d,coeffs:%d,fullrange:%d",primaries,transfer,coeffs,fullRange);
