...
typedef struct {
  ...
  uint32 numRefFrames;
  uint32 has_b_frames;
  int32 need_deinterlace;
  uint8 colorPrimaries;                             // u(8)
  uint8 transfer;                     // u(8)
  uint8 frame_mb_only;
} H264SwDecInfo;
...
typedef int (*FunctionType_MbinfoMallocCB)(void *aUserData,
                                           unsigned int size_mbinfo,
...
MMDecRet H264Dec_GetLastDspFrm(AVCHandle *avcHandle, void **pOutput,
                               int32 *picId, uint64 *pts);
void H264Dec_SetCurRecPic(AVCHandle *avcHandle, uint8 *pFrameY,
                          uint8 *pFrameY_phy, void *pBufferHeader, int32 picId);

int H264Dec_get_iova(AVCHandle *avcHandle, int fd, unsigned long *iova, size_t *size);

int H264Dec_free_iova(AVCHandle *avcHandle, unsigned long iova, size_t size);

int H264Dec_get_IOMMU_status(AVCHandle *avcHandle);
MMDecRet H264Dec_InitStructForNewSeq(AVCHandle *avcHandle);
...
typedef void (*FT_H264Dec_SetCurRecPic)(AVCHandle *avcHandle, uint8 *pFrameY,
                                        uint8 *pFrameY_phy, void *pBufferHeader,
                                        int32 picId);
typedef MMDecRet (*FT_H264Dec_GetLastDspFrm)(AVCHandle *avcHandle,
...
typedef int (*FT_H264Dec_get_iova)(AVCHandle *avcHandle, int fd, unsigned long *iova, size_t *size);

typedef int (*FT_H264Dec_free_iova)(AVCHandle *avcHandle, unsigned long iova, size_t size);

typedef int (*FT_H264Dec_get_IOMMU_status)(AVCHandle *avcHandle);
typedef MMDecRet (*FT_H264Dec_InitStructForNewSeq)(AVCHandle *avcHandle);
