...
typedef struct tagAVCEncParam {
  ...
  int *offset_poc_ref;        /* array of offset for ref pic, dimension
                                 [num_ref_in_cycle] */
  ...
  int idr_period;   /* idr frame refresh rate in number of target encoded frame
...
MMEncRet H264EncInit(AVCHandle *avcHandle, MMCodecBuffer *pExtaMemBfr,
                     MMCodecBuffer *pBitstreamBfr, MMCodecBuffer *pBitstreamBfrPn,
...
int H264Enc_get_iova(AVCHandle *avcHandle, int fd, unsigned long *iova, size_t *size);

int H264Enc_free_iova(AVCHandle *avcHandle, unsigned long iova, size_t size);

int H264Enc_get_IOMMU_status(AVCHandle *avcHandle);
...
typedef int (*FT_H264Enc_get_iova)(AVCHandle *avcHandle, int fd, unsigned long *iova, size_t *size);

typedef int (*FT_H264Enc_free_iova)(AVCHandle *avcHandle, unsigned long iova, size_t size);

typedef int (*FT_H264Enc_get_IOMMU_status)(AVCHandle *avcHandle);
