...
SPRDVP9Decoder::SPRDVP9Decoder(
    ...
    ALOGI("Construct SPRDVP9Decoder, this: %p,instances: %d", (void *)this, instances);
    ...
    property_get("vendor.vp9dec.yuv.dump", value_dump, "false");
    mDumpYUVEnabled = !strcmp(value_dump, "true");

    property_get("vendor.vp9dec.strm.dump", value_dump, "false");
    ...
    if(mDumpStrmEnabled){
        char s2[100];
        sprintf(s2,"/data/misc/media/video_es_%p_%lld.vp9",(void *)this,start_decode);
...
OMX_ERRORTYPE SPRDVP9Decoder::initCheck() const{
...
void SPRDVP9Decoder::initPorts() {
    ...
    def.nBufferSize = mInputBufferSize;
...
status_t SPRDVP9Decoder::initDecoder() {
    ...
    mInputBufferSize = mMaxWidth*mMaxHeight*3/2/4;
...
void SPRDVP9Decoder::onQueueFilled(OMX_U32 portIndex) {
    ...
    while ((mEOSStatus != INPUT_DATA_AVAILABLE || !inQueue.empty())
        ...
        ALOGV("bufferSize %d, %d",bufferSize,inHeader->nFilledLen);
...
OMX_ERRORTYPE SPRDVP9Decoder::setConfig(
    OMX_INDEXTYPE index, const OMX_PTR params) {
    switch (index) {
